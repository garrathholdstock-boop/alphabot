...
FILES_TO_BACKUP = [
    ...
    ("data/universe_loader.py",      "universe_loader.py"),
    ("data/watchlist_refresh.py",    "watchlist_refresh.py"),
    ("core/config.py",               "config.py"),
    ("core/execution.py",            "execution.py"),
    ("ai_debug/main.py",             "agent_main.py"),
    ("refresh_universe_cli.py",      "refresh_universe_cli.py"),
...
<div class="card" style="margin-bottom:20px;border-color:rgba(245,158,11,0.2)">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px">
    ...
    <div style="background:#0a1020;border:1px solid rgba(0,170,255,0.3);border-radius:10px;padding:16px">
      ...
      <div style="font-size:12px;color:#94a3b8;margin-bottom:14px;line-height:1.6">
        Full rebuild of all 6 watchlists from index constituents
        (<strong style="color:#f8fafc">S&amp;P 500/400/600, NASDAQ 100, FTSE 100/250, ASX 200/300</strong>)
        cross-referenced against IBKR's live tradeable universe.
        Takes 2–5 minutes. Current lists archived. Bot restart required. PIN needed.
      </div>
      <button id="uv-btn" onclick="refreshUniverse()"
        class="btn" style="width:100%;background:rgba(0,170,255,0.1);border:1px solid rgba(0,170,255,0.4);color:#00aaff">
...
function refreshUniverse() {{
  ...
  result.style.display = 'none';
  prog.style.width = '5%';
  prog.style.background = 'linear-gradient(90deg,#00aaff,#60c4ff)';
  ...
  .then(function(d) {{
    ...
    var poll = setInterval(function() {{
      ...
      fetch(BASE+'/maintenance/refresh-universe/status')
        ...
        .catch(function(e) {{ /* agent may be restarting — keep polling */ }});
    }}, 3000);
  }})
  .catch(function(err) {{
...
# ──────────────────────────────────────────────────────────────
# Universe + watchlist refresh — background job
# ──────────────────────────────────────────────────────────────
...
async def refresh_universe_endpoint(request: Request):
    """PIN-gated full universe scrape + watchlist rebuild.
    Runs in a background thread — returns immediately.
    Progress tracked via module-level _universe_refresh_state."""
    from fastapi.responses import JSONResponse as JR
    import threading
    try:
        body = await request.json()
        ...
        def _run():
            ...
            try:
                ...
                _universe_refresh_state["message"] = (
                    f"✅ Universe: {u['counts'].get('universe', 0)} symbols • "
        ...
        return JR({"status": "ok", "message": "🌍 Universe refresh started — 2-5 min. Progress below."})
    except Exception as e:
...
async def refresh_universe_status():
    ...
    from fastapi.responses import JSONResponse as JR
    state = dict(_universe_refresh_state) if _universe_refresh_state else {"status": "idle"}
