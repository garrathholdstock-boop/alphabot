...
@media(max-width:599px){.scan-row-desktop{display:none!important}}
    ...
    crypto_id_scored     = score_candidates(st_candidates.get("crypto_id", []))
...
<div id="{rid}_mob_detail" style="display:none;padding:10px 12px;font-size:12px;color:#b0bec5;
  ...
  SCR:<b style="color:#e0e0e0">{v_scr}</b> · EMA:<b style="color:#e0e0e0">{v_ema}</b> · RSI:<b style="color:#e0e0e0">{v_rsi}</b> · Vol:<b style="color:#e0e0e0">{v_vol}</b> · VAP:<b style="color:#e0e0e0">{v_vap}</b>{"<span style=\"color:#00ff88;font-weight:700\"> ✅</span>" if all_pass else ""}
    ...
    rtt_rows = (
        ready_to_trade_rows(crypto_id_scored, "#aa88ff", "Crypto Intraday", _held_syms) +
        ready_to_trade_rows(crypto_swing_scored, "#00ff88", "Crypto Swing", _held_syms) +
    ...
    if rtt_rows:
        ...
        ready_to_trade_html = (
            ...
            f'<div style="font-size:13px;color:#94a3b8">No signals qualify right now — watching {sum(len(x) for x in [us_scored,crypto_id_scored,crypto_swing_scored,ftse_scored,asx_scored,sc_scored])} stocks across all markets</div>'
            f'</div></div>'
        )
...
@media(max-width:599px){.scan-row-desktop{display:none!important}}
    ...
    market_scanner_html = (
        ...
        f'</div>'
        + build_market_accordion("crypto_id","Crypto Intraday","⚡","#aa88ff", True, crypto_id_scored, True)
        + build_market_accordion("crypto_swing","Crypto Swing","🪙","#00ff88", True, crypto_swing_scored, True)
...
<div class="pstrip" style="display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-bottom:14px">
  ...
  <div class="card">
    ...
    <div style="font-size:12px;color:#94a3b8;margin-top:2px">Trades <b style="color:#f8fafc">{d0["t"]}</b></div>
    <div style="font-size:12px;color:#94a3b8;margin-top:3px">Win <b style="color:{_wrcol(d0['wr'])}">{d0["wr"]}%</b> &nbsp;·&nbsp; Avg <b style="color:{_col(d0["avg"])}">{_fmt(d0["avg"])}</b></div>
  </div>
  <div class="card">
    ...
    <div style="font-size:12px;color:#94a3b8;margin-top:2px">Trades <b style="color:#f8fafc">{d1["t"]}</b></div>
    <div style="font-size:12px;color:#94a3b8;margin-top:3px">Win <b style="color:{_wrcol(d1['wr'])}">{d1["wr"]}%</b> &nbsp;·&nbsp; Avg <b style="color:{_col(d1["avg"])}">{_fmt(d1["avg"])}</b></div>
  </div>
  <div class="card">
    ...
    <div style="font-size:12px;color:#94a3b8;margin-top:2px">Trades <b style="color:#f8fafc">{d2["t"]}</b></div>
    <div style="font-size:12px;color:#94a3b8;margin-top:3px">Win <b style="color:{_wrcol(d2['wr'])}">{d2["wr"]}%</b> &nbsp;·&nbsp; Avg <b style="color:{_col(d2["avg"])}">{_fmt(d2["avg"])}</b></div>
  </div>
  <div class="card">
    ...
    <div style="font-size:12px;color:#94a3b8;margin-top:2px">Trades <b style="color:#f8fafc">{week_t}</b></div>
    <div style="font-size:12px;color:#94a3b8;margin-top:3px">Win <b style="color:{_wrcol(week_wr)}">{week_wr}%</b> &nbsp;·&nbsp; Avg <b style="color:{_col(week_pnl/week_t if week_t else 0)}">{_fmt(week_pnl/week_t if week_t else 0)}</b></div>
  </div>
  <div class="card">
    ...
    <div style="font-size:12px;color:#94a3b8;margin-top:2px">Trades <b style="color:#f8fafc">{total_t}</b></div>
    <div style="font-size:12px;color:#94a3b8;margin-top:3px">Win <b style="color:{_wrcol(win_rate)}">{win_rate}%</b> &nbsp;·&nbsp; Avg Score <b style="color:#f8fafc">{avg_sc_db:.1f}</b></div>
  </div>
</div>
...
<div class="container" style="max-width:860px">
  ...
  <form id="settings-form">
    ...
    <div class="settings-section">
      ...
      {row("Stop Loss %", "STOP_LOSS_PCT", 5.0, step="0.1", note="Swing trades — fallback if ATR unavailable")}
    ...
    <div class="settings-section">
      <div class="settings-section-title">📐 ATR Dynamic Stops</div>
      <div style="font-size:12px;color:#94a3b8;margin-bottom:14px;line-height:1.6">
        ATR (Average True Range) sets stop loss based on each stock's actual volatility.
        Stop = Entry − (ATR × Multiplier). A higher multiplier = wider stop = fewer premature exits.
        Falls back to fixed Stop Loss % if ATR data unavailable.
      </div>
      {row("ATR Stop Multiplier", "ATR_STOP_MULTIPLIER", 2.0, step="0.1", note="Stop distance in ATR units. 2.0 = 2× ATR below entry")}
      {row("Use ATR Stops", "USE_ATR_STOPS", 1, step="1", note="1 = enabled, 0 = use fixed % only")}
    </div>
