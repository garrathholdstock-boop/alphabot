...
def is_asx_open():
    """ASX: Mon-Fri 10:00-16:00 Sydney local time.
    Uses Australia/Sydney timezone, so DST (AEDT/AEST) handled automatically."""
    now = datetime.now(ZoneInfo("Australia/Sydney"))
    if now.weekday() >= 5: return False
    mins = now.hour * 60 + now.minute
    return 600 <= mins < 960   # 10:00 .. 16:00

def is_ftse_open():
    """LSE: Mon-Fri 08:00-16:30 London local time.
    Uses Europe/London timezone, so DST (GMT/BST) handled automatically."""
    now = datetime.now(ZoneInfo("Europe/London"))
    if now.weekday() >= 5: return False
    mins = now.hour * 60 + now.minute
    return 480 <= mins < 990   # 08:00 .. 16:30
